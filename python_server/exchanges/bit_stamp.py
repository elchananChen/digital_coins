...
# 1. √ errors for each coin 
# 2. √ new relevant payloads

# 3. √ success insertion 
# 4. √ start time for each coin 
# 5. √ time to insert
...
async def save_to_db(data, exchange_name, exchange_symbol):
    ...
    try:
        ...
        seconds = insert_time.total_seconds()


        update_tracker(save_to_db_time_tracker, exchange_symbol, insert_time=seconds)
        update_tracker(insert_tracker, exchange_symbol,f"{exchange_symbol} first insert")
    except Exception as e:
        print(f"Error inserting data to DB for {exchange_name} {exchange_symbol}: {e}")
        traceback.print_exc()

async def get_bit_stamp_coin_order_book(bit_stamp_symbol, db_symbol, context, sleep_time=0):
    try:
        ...
        def on_websocket(ws):
                ...
                async def process_data():
                    ...
                    # print (data)
                    await save_to_db(exchange_name="bit_stamp", data=data, exchange_symbol=bit_stamp_symbol)
...
async def run_bit_stamp_scraper(context):
    tasks = []
    ...
    for i, (db_symbol,bit_stamp_symbol) in enumerate(bit_stamp_symbols.items()):
        ...
        task = asyncio.create_task(safe_wrapper(get_bit_stamp_coin_order_book(bit_stamp_symbol, db_symbol, context, sleep_time)))
        tasks.append(task)
    try:
